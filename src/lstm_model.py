...
def load_data(path="data/processed/prices.csv"):
    ...
    if len(df) <= WINDOW:
        print(f"ERROR: not enough rows ({len(df)}) for window size {WINDOW}")
        sys.exit(1)
...
def save_metrics(metrics, path="data/processed/lstm_metrics.json"):
    try:
        json.dump({"model": "LSTM", **metrics}, open(path, "w"))
    except Exception as e:
